...
const SurveyWall = ({intl, survey, onGoToClick}) =>{
  ...
  const bodyText = <span>{intl.formatMessage(messages.newSurvey)}: <span onClick={onGoToClick.bind(this,'survey/'+ survey.object_id)} className="text-important">{survey.extra_data.title}.{survey.extra_data.ext}</span></span>
    return(
      <FeedItem
        ...
        expander={
          <ActionsRow>
            <Button onClick={onGoToClick.bind(this, 'survey/'+survey.object_id)} primary flat>{intl.formatMessage(messages.fill)}</Button>           
          </ActionsRow>
        }
    />
    )
